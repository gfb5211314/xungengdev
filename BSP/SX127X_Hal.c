...
#define  rcc522_spi     hspi2
#define  DIO0_PORT       LORA_DIO0_GPIO_Port
...
{
    GPIO_PinState State;
    State = HAL_GPIO_ReadPin(DIO0_PORT, DIO0_PIN);
    return State;
}
/**
...
{
    unsigned char RxDat;
    HAL_SPI_TransmitReceive(&rcc522_spi, &data, &RxDat, 1, 1000);
    return RxDat;
}
/**
  * @简介：SX127X  向寄存器地址连续发送数据
