#include "speak_hal.h"
#define  BELL_EN_Pin    BEEP_SD_Pin
#define  BELL_EN_pord BEEP_SD_GPIO_Port
#define  speak_data_Pin    DATA_Pin
#define  speak_data_GPIO_Port    DATA_GPIO_Port
...
uint8_t read_music_pin()
{
	  ...
	  return  HAL_GPIO_ReadPin(BUSY_SEPAK_GPIO_Port, BUSY_SEPAK_Pin);
}
...
//	HAL_Delay(2);
  ...
  HAL_GPIO_WritePin(BELL_EN_pord,  BELL_EN_Pin, GPIO_PIN_SET);  //
	 HAL_GPIO_WritePin(speak_data_GPIO_Port, speak_data_Pin, GPIO_PIN_SET);  //
uint8_t S_DATA,j;
uint8_t B_DATA; 
S_DATA = DDATA;
sda_L;
