...
void  ETH_UsartReceive_IDLE()
{
    ...
    if((__HAL_UART_GET_FLAG(&eth_usart,UART_FLAG_IDLE) != RESET))
    {
        ...
        temp = eth_usart.hdmarx->Instance->NDTR;//	
...
//  	USART2->RDR = data;
	  HAL_UART_Transmit(&eth_usart, (uint8_t *)&data, 1, 0xffff);
}
//发送包头
...
//							
//			}
			data=strstr((const char*)ether_st.RX_pData,(const char*)str);
			if(data)
				return (u8*)data;	
		}
	}
...
//		}
		if(//判断是不是模块返回的应答包				
					ether_st.RX_pData[0]==0XEF
				&&ether_st.RX_pData[1]==0X01
				&&ether_st.RX_pData[6]==0X07
			)
			{
				*PS_Addr=(ether_st.RX_pData[2]<<24) + (ether_st.RX_pData[3]<<16)
								+(ether_st.RX_pData[4]<<8) + (ether_st.RX_pData[5]);
			ether_st.RX_flag=0;
				return 0;
			}
			ether_st.RX_flag=0;					
